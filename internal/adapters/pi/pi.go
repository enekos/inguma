// Package pi implements the Adapter interface for the Pi coding agent
// (github.com/badlogic/pi-mono). Pi is an opinionated minimal agent that
// does not host MCP servers in its core, but the community `pi-mcp-adapter`
// reads MCP configuration from ~/.pi/agent/mcp.json using the standard
// `mcpServers` schema. We write there so Pi users who have pi-mcp-adapter
// installed pick the tool up automatically.
package pi
...
func (a *Adapter) DisplayName() string { return "Pi" }
...
func (a *Adapter) Snippet(m manifest.Tool) (snippets.Snippet, error) {
	if m.Kind != manifest.KindMCP {
		return snippets.Snippet{
			...
			Content:     "# Pi hosts MCP servers via pi-mcp-adapter. Use the CLI one-liner tab for CLI tools, or drop skills/prompts into ~/.pi/agent/.",
		}, nil
	}
...
func readOrEmpty(path string) (map[string]any, error) {
	...
	if err := json.Unmarshal(data, &cfg); err != nil {
		return nil, fmt.Errorf("pi: parse %s: %w", path, err)
	}
	return cfg, nil
}
...
func writeAtomic(path string, cfg map[string]any, backupDir string) error {
	...
	if backupDir != "" {
		if prev, err := os.ReadFile(path); err == nil {
			...
			_ = os.WriteFile(filepath.Join(backupDir, "pi."+stamp+".json"), prev, 0o644)
		}
	}
